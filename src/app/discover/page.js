...
function StateDrawer({ open, onSelect }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-8 relative">
        <h2 className="text-xl font-bold mb-4 text-center">Select a State</h2>
        <div className="grid grid-cols-2 gap-3 max-h-96 overflow-y-auto">
          {INDIAN_STATES.map(state => (
            <button
              key={state}
              className="px-4 py-2 bg-accentYellow text-borderDark rounded-lg font-medium hover:bg-yellow-400 transition-colors"
...
function Discover() {
  ...
  // When a state is selected, set filter and close drawer
  ...
  return (
    <>
      <StateDrawer open={showStateDrawer} onSelect={handleStateSelect} />
